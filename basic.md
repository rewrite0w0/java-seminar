...
| --------- | --------------- | -------------------------------------- |
| 명사      | 동사            | 의미있게                               |
| CamelCase | 첫글자만 소문자 | `#`, `$` 등 특수문자를 사용하지 않는다 |
...
예를 들어 표기방법이 없었을 때, `10`은 10진수의 10으로도 보이지만, 2진수의 1, 0 일 수도 있다.
...
- try-catch는 finally에 의해 구문을 닫아줘야하는 것이 안전
- try-catch에서 with 사용하면 구문을 자동적으로 닫히게끔 작성
...
try {
    ...
    // Error 는 Java VM 에러 등 심각한 상태를 표현하기 위함.
    // 특별한 이유가 아니라면 캐치할 것이 아님
    ...
}
```
...
- 이하의 정렬되지 않은 숫자 배열을 정렬한 뒤에, 2의 배수 중 10보다 큰 숫자만 남기고 거기에 2배를 하고 싶을 때 `.` 첨자로 데이터 흐름을 정리한다.
...
- 자바도 이러한 동작을 할 수 있음
- 이하는 배열의 중복 제거 후에 짝수만 걸러서 배열 반환
